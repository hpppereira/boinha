...
DURACAO_MINIMA_GRUPO = 30  # segundos
...
def calcula_grupos(signal, fs=1, threshold_percentile=25):
    ...
    for i in range(len(inicios)):
        ...
        intervalo = (ini / fs - fim_anterior / fs) if fim_anterior is not None else np.nan
        fim_anterior = fim
...
def atualizar(frame):
    ...
    for i, g in enumerate(stats["grupos"]):
        ...
        # Rótulo na parte inferior com Hs e intervalo
        t_centro = (t_rel[g["ini"]] + t_rel[g["fim"]]) / 2
        y_txt = ax.get_ylim()[0] + 0.05  # próximo da base do gráfico
        texto = f"G{i+1}\nHs={g['Hs']:.2f} m"
        if not np.isnan(g["intervalo"]):
            texto += f"\nΔt={g['intervalo']:.1f}s"
        ax.text(t_centro, y_txt, texto,
    ...
    int_med = np.nanmean([g["intervalo"] for g in stats["grupos"][1:]]) if n > 1 else 0

    texto_stats.set_text(
        f"Nº grupos (>30s): {n}\nDur. média: {dur_med:.1f} s\n"
        f"Hs médio: {Hs_med:.2f} m\nΔt médio: {int_med:.1f} s"
    )
