...
def calcula_grupos(signal, fs=1, threshold_percentile=25):
    ...
    for i in range(len(inicios)):
        ...
        # Altura significativa do grupo (4*desvio padrão)
...
def atualizar(frame):
    ...
    # Preenche grupos com cor proporcional à duração
    for i, g in enumerate(stats["grupos"]):
        ...
        # Adiciona rótulo “G1”, “G2” etc.
        t_centro = (t_rel[g["ini"]] + t_rel[g["fim"]]) / 2
        y_txt = ax.get_ylim()[1] * 0.85
        ax.text(t_centro, y_txt, f"G{i+1}\nHs={g['Hs']:.2f} m", ha='center', va='top', fontsize=9, color='black')
