...
try:
    ...
    with open(csv_filename, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["date", "wall_time", "sensor_type", "timestamp_ns", "accuracy", "x", "y", "z"])
...
def on_message(ws, message):
    ...
    wall_time = time.time()
    date = pd.to_datetime(wall_time, unit='s') - timedelta(hours=3)

    row = [date, wall_time, sensor_type, timestamp_ns, accuracy, values[0], values[1], values[2]]
